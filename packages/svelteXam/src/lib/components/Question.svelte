...
>
  ...
  <!--qn body-->
  {#if question.body}
    <div class="content" class:noMarks={!question.marks}>
      {@html contentHandler(question.body)}
    </div>
  {/if}
  {#if question.marks}
  ...
  {#each question.parts ?? [] as part, i}
    ...
    <section class="part" aria-label={id} {id}>
      {#if link}
        <a class="part-label" href={`#${link}${qnNo}${partLabel}`}
          >({partLabel})</a
        >
      {:else}
      ...
      {/if}
      {#if part.body}
        <div class="content" class:noMarks={!part.marks}>
          {@html contentHandler(part.body)}
        </div>
      {/if}
      {#if part.marks}
      ...
      {#each part.parts ?? [] as subpart, j}
        ...
        <section class="subpart" aria-label={id} {id}>
          {#if link}
            <a
              class="subpart-label"
              href={`#${link}${qnNo}${partLabel}${subpartLabel}`}
              >({subpartLabel})</a
            >
          {:else}
          ...
          {/if}
          {#if subpart.body}
            <div class="content" class:noMarks={!subpart.marks}>
              {@html contentHandler(subpart.body)}
            </div>
          {/if}
          {#if subpart.marks}
